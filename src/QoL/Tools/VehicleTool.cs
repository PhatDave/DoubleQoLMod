...
namespace DoubleQoL.QoL.Tools {
    ...
    internal class VehicleTool : ASelectableEntityTool {
        ...
        private readonly Lyst<VehicleTypeInfo> vehicleTypeInfos = new Lyst<VehicleTypeInfo>() {
            ...
            new VehicleTypeInfo("Excavator", DoubleQoLShortcutsMap.Instance.VehicleExcavatorToolKb,
                Ids.Vehicles.ExcavatorT1, Ids.Vehicles.ExcavatorT2, Ids.Vehicles.ExcavatorT3),
            new VehicleTypeInfo("Tank Truck", DoubleQoLShortcutsMap.Instance.VehicleTankToolKb,
                Ids.Vehicles.TruckT3Loose.Id),
            new VehicleTypeInfo("Dump Truck", DoubleQoLShortcutsMap.Instance.VehicleDumpToolKb,
        ...
        public VehicleTool(
            ...
            UiBuilder builder) :
            base(protosDb, unlockedProtosDb, shortcutsManager, inputManager, cursorPickingManager, cursorManager,
                areaSelectionToolFactory, entitiesManager, highlighter, toolbarController, terrainCursor, builder) {
            _vehiclesManager = vehiclesManager;
            Vehicles = new Lyst<Vehicle>();
            MovedVehicles = new Lyst<Vehicle>();
            ClearSelectionOnDeactivateOnly();
            Lyst<ToolToggleBtn> toolToggleBtns = new Lyst<ToolToggleBtn>() {
                new ToolToggleBtn("Select", IconPaths.Tool_Select, _ => { }, shortcutsManager.PrimaryAction,
                    "Select and move vehicles", true)
            };
            vehicleTypeInfos.ForEach(vti =>
                toolToggleBtns.Add(new ToolToggleBtn(vti.Name, vti.IconPath, _ => { }, vti.Trigger,
                    $"Select/Move {vti.Name}s")));
            toolToggleBtns.Add(new ToolToggleBtn("Clear truck cargo",
                Mafi.Unity.Assets.Unity.UserInterface.General.Trash128_png, _ => { },
                DoubleQoLShortcutsMap.Instance.ClearTruckCargoToolKb, "Try to clear the cargo of selected trucks"));
            toolToggleBtns.Add(new ToolToggleBtn("Recover vehicle",
                Mafi.Unity.Assets.Unity.UserInterface.General.RecoverVehicle_svg, _ => { },
                DoubleQoLShortcutsMap.Instance.RecoverVehicleToolKb, "Recover the selected vehicles"));
            toolToggleBtns.Add(new ToolToggleBtn("Upgrade vehicle",
                Mafi.Unity.Assets.Unity.UserInterface.General.RecoverVehicle_svg, _ => { },
                DoubleQoLShortcutsMap.Instance.UpgradeVehicleToolKb, "Upgrade the selected vehicles"));
        ...
        public void HandleInput(IInputScheduler inputScheduler) {
            if (ShortcutsManager.IsPrimaryActionUp && IsClick &&
                m_terrainCursor.TryComputeCurrentPosition(out var position))
                HandleMoveAllVehicles(inputScheduler, position);
            if (DoubleQoLShortcutsMap.Instance.ClearTruckCargoToolKb.IsPrimaryOn(IgnoreModifiers))
                HandleCLearAllCargos(inputScheduler);
            if (DoubleQoLShortcutsMap.Instance.RecoverVehicleToolKb.IsPrimaryOn(IgnoreModifiers))
                HandleRecoverAllVehicles(inputScheduler);
            if (DoubleQoLShortcutsMap.Instance.UpgradeVehicleToolKb.IsPrimaryOn(IgnoreModifiers))
                HandleUpgradeAllVehicles(inputScheduler);
        }
        ...
        private void HandleCLearAllCargos(IInputScheduler inputScheduler) =>
            GetVehicles().ForEach(v => TryClearCargo(inputScheduler, v));

        private void HandleRecoverAllVehicles(IInputScheduler inputScheduler) =>
            GetVehicles().ForEach(v => TryRecoverVehicle(inputScheduler, v));

        private void HandleUpgradeAllVehicles(IInputScheduler inputScheduler) {
            GetVehicles().ForEach(v => TryUpgradeVehicle(inputScheduler, v));
        }
        ...
        public void TryRecoverVehicle(IInputScheduler inputScheduler, Vehicle v) =>
        ...
        private Lyst<VehicleTypeInfo> GetActiveVehicleTypeInfo() =>
            vehicleTypeInfos.FindAll(e => e.Trigger.IsPrimaryOn(IgnoreModifiers));
        ...
        protected override void OnEntitiesSelected(IIndexable<IAreaSelectableEntity> selectedEntities,
            IIndexable<SubTransport> selectedPartialTransports, bool isAreaSelection, bool isLeftClick) {
        ...
        public override void RegisterUi(UiBuilder builder) {
            if (ConfigManager.Instance.QoLs_vehicletool.Value) {
                _toolbarController
                    .AddLeftMenuButton(
                        Loc.Str("VehicleTool", "Vehicle Tool",
                            "title of a tool that is used to select and move vehicles").TranslatedString, this,
                        Mafi.Unity.Assets.Unity.UserInterface.Toolbar.Vehicles_svg, 80f,
                        m => DoubleQoLShortcutsMap.Instance.VehicleToolKb)
                    .AddTooltip(new LocStrFormatted("Select One or multiple vehicles to move them"));
                InitializeUi(builder,
                    new Mafi.Unity.UiFramework.Styles.CursorStyle(
                        nameof(Mafi.Unity.Assets.Unity.UserInterface.Toolbar.Vehicles_svg),
                        Mafi.Unity.Assets.Unity.UserInterface.Toolbar.Vehicles_svg, new UnityEngine.Vector2(14f, 14f)),
                    builder.Audio.ButtonClick, COLOR_HIGHLIGHT, COLOR_HIGHLIGHT_CONFIRM);
            ...
            if (ConfigManager.Instance.QoLs_vehicletool.Value) {
                _toolbarController
                    .AddLeftMenuButton(
                        ...
                        m => DoubleQoLShortcutsMap.Instance.VehicleToolKb)
                    .AddTooltip(new LocStrFormatted("Select One or multiple vehicles to move them"));
                InitializeUi(_builder,
                    new Mafi.Unity.UiFramework.Styles.CursorStyle(
                        nameof(Mafi.Unity.Assets.Unity.UserInterface.Toolbar.Vehicles_svg),
                        Mafi.Unity.Assets.Unity.UserInterface.Toolbar.Vehicles_svg, new UnityEngine.Vector2(14f, 14f)),
                    _builder.Audio.ButtonClick, COLOR_HIGHLIGHT, COLOR_HIGHLIGHT_CONFIRM);
            }
        }
